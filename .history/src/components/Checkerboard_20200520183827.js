...
class Checkeredboard extends Component{
    ...
    moveCell = (id) =>{
        const move = this.state.startArray;
        const itemcell = move.find(item => item.id === id);
        const itemIndexcell = move.indexOf(itemcell);
        move[itemIndexcell]['piece'] = 3;
        this.setState({
            startArray: itemIndexcell,
