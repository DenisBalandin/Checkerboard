...
class Checkeredboard extends Component{
    ...
    componentDidMount = () =>{
        ...
        if(this.state.nextPlayer === 0){
            this.setState({
                matrixArray:JSON.parse(localStorage.getItem('saveGame')),
                nextPlayer:2
            });
        }else{
            this.setState({
                matrixArray:localStorage.getItem('saveGame')
            });
        }
    ...
    saveGame = () =>{
        localStorage.setItem('saveGame',  JSON.stringify(this.state.matrixArray));
    }
    render(){
