...
class Checkeredboard extends Component{
    ...
    movetToShape = (id, type) =>{
        if(id !== this.state.shapeMove.id ){
            ...
            if(typeof this.state.shapeMove.id !== 'undefined'){
                console.log(this.state.shapeMove.type);
                itemsst[itemIndex]['piece'] = this.state.shapeMove.piece;
                this.setState({
                    startArray: itemsst,
                }); 
            }
    ...
    moveCell = (id,type) =>{
        ...
        this.setState({
            startArray: itemsst,
            shapeMove:{id:id,piece:3,type:3},
            cellMove:[{idleft:'',idrighr:''}]
        }); 
                        ...
                        <div className="boardRow">{number.map((a) =>
                            <div>{a.cell == 0 ? 
                                <div className="blackcels">
                                    {a.id === this.state.cellMove.idleft ? <div className="movecell" onClick={this.moveCell.bind(this, a.id,a.piece)}></div>: <div></div> }
                                    {a.id === this.state.cellMove.idrighr ? <div className="movecell" onClick={this.moveCell.bind(this, a.id,a.piece)}></div>: <div></div> }
